# SnapCC Effective File System Consistency Testing Using
...
⇒ 크래시 복구 메커니즘 설계 hard, 충분한 검증이 없을 경우 치명적인 데이터 손실 ㅣ버그가 실제 배포된 소프트웨어에 남아있을 위협이 큼 
...
1. 모든 온디스크 상태를 체계적으로 탐색하는 문제와 valid 상태를 자동으로 찾는 문제는 사실 같은 문제
    - 영속성 연산 : 메타데이터/데이터 블록을 갱신하기 위한 블록 쓰기의 연속과정
    - 호출 시퀀스 실행 중
